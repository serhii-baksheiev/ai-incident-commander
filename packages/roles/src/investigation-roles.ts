...
import { describeStatusRules } from './status-rules-prompt.js';
 ...
 * run's own read of its hypotheses rather than a second, competing one. The
 * system prompt also carries what each of those derived statuses MEANS
 * (owner ruling D1, item 6): one sentence per status, generated by
 * `describeStatusRules` (`./status-rules-prompt.js`) from
 * `STATUS_RULES[STATUS_RULES_VERSION]` (`@aic/domain`), so a status the model
 * has not seen defined before — `corroborated` — is never shown as a bare
 * word.
...
export function createModelProposeConclusion(
  ...
  // AIC-119 slice 5 (owner ruling D1, item 6): what `derived hypothesis
  // statuses` below actually MEANS, generated from the live status-rules
  // table rather than left for the model to guess from the bare word.
  // see conclusion-role.test.mjs › "propose_conclusion's system prompt contains every sentence describeStatusRules(STATUS_RULES[STATUS_RULES_VERSION]) returns"
  const statusRuleSentences = Object.values(
