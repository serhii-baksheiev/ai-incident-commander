...
// This module carries the persistence spike exactly as it ran before AIC-99:
// same options, same outputs, same tracing behaviour. It has moved from the
// CLI's top level to `aic dev spike` (docs/decisions/integration-boundary.md,
// "Terminology") without changing a single code path.

export const devSpikeHelp = `Usage: aic dev spike <command> [options]

Commands:
  start    Start and checkpoint a persistent investigation spike
  resume   Resume a checkpointed run by its run id

Run "aic dev spike <command> --help" for command options.`;

function commandHelp(command: 'start' | 'resume'): string {
  return `Usage: aic dev spike ${command} --run-id <id> --checkpoint <sqlite-file>
...
export async function runDevSpike(args: readonly string[]): Promise<void> {
  const [command, ...commandArgs] = args;
  if (command === undefined || command === '--help' || command === '-h') {
    process.stdout.write(`${devSpikeHelp}\n`);
    return;
  }
  if (command !== 'start' && command !== 'resume') {
    throw new Error(`unknown command: dev spike ${command}`);
  }
