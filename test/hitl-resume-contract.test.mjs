 ...
 *
 * `LogicalCountSchema` is not what guards these two ON THIS PATH. Since AIC-76
 * both fields carry it, so a `-1` is refused at the `kind: 'start'` boundary —
 * but a `kind: 'resume'` takes its state from the checkpointer and is never
 * parsed by `IncidentStateSchema` at all, which is what these rows are about.
 * What refuses such a value here is the graph's own
